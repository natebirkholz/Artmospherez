...
class AnimateToWeatherDetailController: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let toViewController = transitionContext.viewController(forKey: .to) as? DetailViewControllerWeather else { return }
        let containerView = transitionContext.containerView
        let duration = transitionDuration(using: transitionContext)

        guard let selectedRow = fromViewController.tableView.indexPathForSelectedRow else { return }
        guard let selectedCell = fromViewController.tableView.cellForRow(at: selectedRow) as? WeatherCell else { return }
